...
1. Scan `source/` for files without corresponding `filename.ext.metadata.json`, or files with metadata `status: pending` that have been updated with new content.
...
- Do not process files that already have a metadata sidecar unless the status is `new` or `pending`.
