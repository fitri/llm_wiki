...
2. **Normalize the source filename** to lowercase kebab-case. Rename the source file if its name does not follow the naming convention. Do not modify file content.
3. For each new file, generate a metadata sidecar using the template at `.system/templates/file.metadata.json`. The metadata filename must match the normalized source filename.
4. Classify `type` based on file extension and content.
